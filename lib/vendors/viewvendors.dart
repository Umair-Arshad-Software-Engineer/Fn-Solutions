...
class _ViewVendorsPageState extends State<ViewVendorsPage> {
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    try {
                      ...
                      // after the switch statement — wrap it:
                      if (selectedPaymentMethod != 'Cheque') {
  ...
  async {
    ...
    final cashbookEntry = {
      ...
      'description': paymentData['description'] != null && paymentData['description'].toString().isNotEmpty
          ? paymentData['description']
          : 'Vendor Payment',
      'dateTime': paymentData['date'],    // ✅ correct key
      'type': 'cash_out',
    };
  ...
  async {
    ...
    final bankTransaction = {
      ...
      'description': paymentData['description'] != null && paymentData['description'].toString().isNotEmpty
          ? paymentData['description']
          : 'Vendor Payment',
      'timestamp': DateTime.now().millisecondsSinceEpoch,  // ✅ number, as rules require
      'type': 'cash_out',
    };
  ...
  void _navigateToEditVendor(String vendorId) async {
    try {
      // Fetch complete vendor data from Firebase
      final vendorSnapshot = await _databaseRef.child(vendorId).get();

      if (vendorSnapshot.value != null) {
        final vendorData = Map<String, dynamic>.from(vendorSnapshot.value as Map<dynamic, dynamic>);

        // Add the vendor ID to the data
        vendorData['id'] = vendorId;

        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => AddVendorPage(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(languageProvider.isEnglish
              ? 'Error loading vendor data: $e'
              : 'وینڈر کے ڈیٹا لوڈ کرنے میں خرابی: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      onTap: () => _navigateToEditVendor(vendor["id"]),
                      trailing: SizedBox(
                        width: 170,
                        child: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () => Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => VendorItemWiseLedgerPage(
                                    vendorId: vendor["id"],
