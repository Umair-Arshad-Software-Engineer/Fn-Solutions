...
class _VendorLedgerPageState extends State<VendorLedgerPage> {
      ...
      )
  {
    ...
    // Helper method to get transaction type (same as in UI)
    ...
    // Helper method to get description text (matching UI's description column)
    String _getDescriptionText(Map<String, dynamic> entry) {
      // Use description2 field if available (same as UI)
      ...
      // For purchases, show reference number if available
      if (description == 'Purchase') {
        final refNo = entry['refNo']?.toString() ?? '';
        // if (refNo.isNotEmpty) return 'Ref: $refNo';
        return 'Purchase';
      }
    ...
    // Table header - OPTIMIZED widths to fit all columns
    rows.add(
      pw.Container(
        ...
        child: pw.Row(
          ...
          children: [
            _buildPdfHeaderCell('Date', 50),           // Reduced from 60
            _buildPdfHeaderCell('Transaction', 55),     // Reduced from 70
            _buildPdfHeaderCell('Description', 65),     // Reduced from 80
            _buildPdfHeaderCell('Type', 45),            // Reduced from 50
            _buildPdfHeaderCell('Payment', 50),         // Reduced from 60
            _buildPdfHeaderCell('Bank', 55),            // Reduced from 70
            _buildPdfHeaderCell('Qty', 35),             // Reduced from 40
            _buildPdfHeaderCell('Weight', 40),          // Reduced from 50
            _buildPdfHeaderCell('Debit', 45),           // Reduced from 50
            _buildPdfHeaderCell('Credit', 45),          // Reduced from 50
            _buildPdfHeaderCell('Balance', 55),         // Reduced from 60
          ],
        ),
      ),
    );
    ...
    for (var entry in sortedEntries) {
      ...
      // USE THE NEW HELPER METHODS (matching UI logic)
      final transactionType = _getTransactionType(entry); // "Purchase", "Payment", "Balance"
      final descriptionText = _getDescriptionText(entry); // Uses description2 field
      ...
      // Main transaction row - OPTIMIZED widths
      rows.add(
        pw.Container(
          padding: const pw.EdgeInsets.all(4),        // Reduced padding from 6
          ...
          child: pw.Row(
            ...
            children: [
              ...
              _buildPdfDataCell(descriptionText, 65),
              _buildPdfDataCell(type, 45),
              _buildPdfDataCell(paymentMethod, 50),
              _buildBankCell(bankName, bankLogo, 55),
              _buildPdfDataCell(isPurchase ? quantity.toStringAsFixed(2) : '-', 35),
              _buildPdfDataCell(isPurchase ? weight.toStringAsFixed(2) : '-', 40),
              _buildPdfDataCell(
    ...
    // Add summary row - OPTIMIZED widths
    rows.add(
      pw.Container(
        padding: const pw.EdgeInsets.all(4),
        decoration: pw.BoxDecoration(
        ...
        child: pw.Row(
          ...
          children: [
            ...
            _buildPdfDataCell('', 55), // Empty for Transaction
            _buildPdfDataCell('', 65), // Empty for Description
